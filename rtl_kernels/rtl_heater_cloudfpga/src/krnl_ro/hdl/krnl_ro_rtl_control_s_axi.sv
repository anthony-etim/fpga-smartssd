...
`timescale 1ns/1ps
module krnl_ro_rtl_control_s_axi
#(parameter
...
// 0x44 : reserved
// 0x48 : Data signal of ro_in
//        bit 31~0 - ro_in[31:0] (Read/Write)
// 0x4c : reserved
// 0x50 : Data signal of ro_out
//        bit 31~0 - ro_out[31:0] (Read/Write)
// 0x54 : reserved
...
localparam
   ...
   /* ADDR_D_DATA_0        = 7'h28,
    ...
    ADDR_C_CTRL          = 7'h3c,
   */
    ADDR_LENGTH_R_DATA_0 = 7'h28,
    ADDR_LENGTH_R_CTRL   = 7'h2c,
    ADDR_RO_IN_DATA_0    = 7'h30,
    ADDR_RO_IN_DATA_1    = 7'h34,
    ADDR_RO_IN_DATA_CTRL = 7'h38,
    ADDR_RO_OUT_DATA_0   = 7'h3c,
    ADDR_RO_OUT_DATA_1   = 7'h40,
    ADDR_RO_OUT_DATA_CTRL= 7'h44,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    RDIDLE               = 2'd0,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_ro_in = 64'b0;
    reg  [63:0]                   int_ro_out = 64'b0;
        ...
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                */
                ...
                ADDR_RO_IN_DATA_0: begin
                    rdata <= int_ro_in[31:0];
                end
                ADDR_RO_IN_DATA_1: begin
                    rdata <= int_ro_in[63:32];
                end
                ADDR_RO_OUT_DATA_0: begin
                    rdata <= int_ro_out[31:0];
                end
                ADDR_RO_OUT_DATA_1: begin
                rdata <= int_ro_out[63:32];
                end
            endcase
        end
    end
...
assign ro_in        = int_ro_in;
assign ro_out       = int_ro_out;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RO_IN_DATA_0)
            int_ro_in[31:0] <= (WDATA[31:0] & wmask) | (int_ro_in[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RO_IN_DATA_1)
            int_ro_in[63:32] <= (WDATA[31:0] & wmask) | (int_ro_in[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RO_OUT_DATA_0)
            int_ro_out[31:0] <= (WDATA[31:0] & wmask) | (int_ro_out[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RO_OUT_DATA_1)
            int_ro_out[63:32] <= (WDATA[31:0] & wmask) | (int_ro_out[63:32] & ~wmask);
    end
end
