...
module ro_sensor #(
    ...
    input wire  [C_NUM_CHANNELS_RO-1:0]                   s_tvalid,
    input wire  [C_NUM_CHANNELS_RO-1:0][C_DATA_WIDTH-1:0] s_tdata,
    output wire [C_NUM_CHANNELS_RO-1:0]                   s_tready,
...
logic [C_DATA_WIDTH-1:0]  m_tdata_reg;
...
assign s_tready = m_tready & m_tvalid ? {C_NUM_CHANNELS_RO{1'b1}} : {C_NUM_CHANNELS_RO{1'b0}};
    ...
    if (areset == 1'b1) begin
        ...
        case (state)
            IDLE: begin
                ro_counter_reset <= 1'b1;
                // if start, 
                if (s_tdata == 32'd1) begin // defined start ro address in address.sv
                    counter_clk <= 0;
                    state <= ROMEASURE;
                end else begin
                    state <= IDLE;
            ...
            ROMEASURE: begin
                if (counter_clk < CLOCKS_PER_BIT) begin
                    ...
                    m_tdata_reg <= ro_counter_count; // m_tdata inside 'always' block... we have to define it as reg instead of wire
