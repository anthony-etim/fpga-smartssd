...
// of this module is to multiply three 32-bit values and produce a result AND do whatever RO does.  The values
// are read from one AXI4 memory mapped master, processed and then written out.
//
// Data flow: axi_read_master->fifo[4]->multiplier/RO->fifo[2]->axi_write_master
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
localparam integer LP_NUM_READ_CHANNELS  = 4;
localparam integer LP_NUM_READ_CHANNELS_MULT  = 3;
localparam integer LP_NUM_READ_CHANNELS_RO    = 1;
localparam integer LP_NUM_WRITE_CHANNELS = 2;
...
logic [C_M_AXI_GMEM_ADDR_WIDTH-1:0] ro_in;
logic [C_M_AXI_GMEM_ADDR_WIDTH-1:0] ro_out;
logic [LP_LENGTH_WIDTH-1:0]         length_r;

// Newly added ///////////////////////////////////////////
logic [LP_LENGTH_WIDTH-1:0] wm_ctrl_length;             //
logic [C_M_AXI_GMEM_ADDR_WIDTH-1:0] wm_ctrl_offset;     //
logic wm_ctrl_start;                                    //
logic sent_mul = 1'b0;                                  //
logic wm_done;                                          //
//////////////////////////////////////////////////////////
...
logic [LP_NUM_WRITE_CHANNELS-1:0]                              adder_tvalid;
logic [LP_NUM_WRITE_CHANNELS-1:0]                              adder_tready_n; 
logic [LP_NUM_WRITE_CHANNELS-1:0][C_M_AXI_GMEM_DATA_WIDTH-1:0] adder_tdata;
logic [LP_NUM_WRITE_CHANNELS-1:0]                              wr_fifo_tvalid_n;
logic [LP_NUM_WRITE_CHANNELS-1:0]                              wr_fifo_tready; 
logic [LP_NUM_WRITE_CHANNELS-1:0][C_M_AXI_GMEM_DATA_WIDTH-1:0] wr_fifo_tdata;
...
// Logic slightly modified, but still needs to be verified..DONE
always @(posedge ap_clk) begin 
  if (areset) begin 
    ap_idle <= 1'b1;
  end
...
// Newly added logic...Needs to be verified ////////////////
always @(posedge ap_clk) begin                            //
  if (ap_start_pulse) begin                               //
    wm_ctrl_start <= 1'b1;                                //
    wm_ctrl_offset <= c;                                  //
    wm_ctrl_length <= length_r;                           //
    ap_done <= 1'b0;                                      //
    sent_mul <= 0;                                        //
  end                                                     //
    else if (wm_done == 1'b1 & sent_mul == 1'b0) begin    //
    sent_mul <= 1'b1;                                     //
    wm_ctrl_start <= 1'b1;                                //
    wm_ctrl_offset <= ro_out;                             //
    wm_ctrl_length <= 1'b1;                               //
    ap_done <= 1'b0;                                      //
  end                                                     //
    else if (wm_done == 1'b1 & sent_mul == 1'b1) begin    //
    wm_ctrl_start <= 1'b0;                                //
    sent_mul <= 0;                                        //
    wm_ctrl_offset <= 0;                                  //
    wm_ctrl_length <= 1'b0;                               // 
...
// AXI4-Lite slave
krnl_ro_rtl_control_s_axi #(
  .C_S_AXI_ADDR_WIDTH( C_S_AXI_CONTROL_ADDR_WIDTH ),
  .C_S_AXI_DATA_WIDTH( C_S_AXI_CONTROL_DATA_WIDTH )
) 
inst_krnl_ro_control_s_axi (
  ...
  .ap_ready  ( ap_ready                      ) ,
  //////////////////////////////////////////////////
  .ap_done   ( ap_done                       ) , /// That needs to be verified as well
  //////////////////////////////////////////////////
  .ap_idle   ( ap_idle                       ) ,
  ...
  .ro_in     ( ro_in[0+:C_M_AXI_GMEM_ADDR_WIDTH] ) ,
  .ro_out    ( ro_out[0+:C_M_AXI_GMEM_ADDR_WIDTH] ) ,
  .length_r  ( length_r[0+:LP_LENGTH_WIDTH]  ) 
);

// AXI4 Read Master
krnl_ro_rtl_axi_read_master #( 
...
// Combinatorial Adder
krnl_ro_rtl_multiply #( 
  .C_DATA_WIDTH   ( C_M_AXI_GMEM_DATA_WIDTH ) ,
  .C_NUM_CHANNELS_MULT ( LP_NUM_READ_CHANNELS_MULT ) 
)
inst_adder ( 
  .aclk     ( ap_clk                 ) ,
  .areset   ( areset                 ) ,

  .s_tvalid ( ~rd_fifo_tvalid_n[2:0] ) ,
  .s_tready ( rd_fifo_tready[2:0]    ) ,
  .s_tdata  ( rd_fifo_tdata[2:0]     ) ,

  .m_tvalid ( adder_tvalid[0]      ) ,
  .m_tready ( ~adder_tready_n[0]   ) ,
  .m_tdata  ( adder_tdata[0]       ) 
);
...
ila_0 i_ila_0 (
	...
	.probe1(rd_fifo_tvalid_n[3]), // input wire [0:0]  probe1 
	.probe2(rd_fifo_tready[3] ),   // input wire [0:0]  probe2 
	.probe3(rd_fifo_tdata[3]),    // input wire [63:0] probe3 
	.probe4(adder_tvalid[1]),     // input wire [0:0]  probe4 
	.probe5(adder_tready_n[1]),   // input wire [0:0]  probe5 
	.probe6(adder_tdata[1])       // input wire [31:0] probe6
);
...
ro_sensor #(
    ...
    .STAGES(STAGES),
    // RTL kernel parameters
    .C_DATA_WIDTH   ( C_M_AXI_GMEM_DATA_WIDTH ),
    .C_NUM_CHANNELS_RO ( LP_NUM_READ_CHANNELS_RO )     
) 
inst_ro_sensor (
  .aclk     ( ap_clk               ) ,
  .areset   ( areset               ) ,

  .s_tvalid ( ~rd_fifo_tvalid_n[3] ) ,
  .s_tready ( rd_fifo_tready[3]    ) ,
  .s_tdata  ( rd_fifo_tdata[3]     ) ,

  .m_tvalid ( adder_tvalid[1]      ) ,
  .m_tready ( ~adder_tready_n[1]   ) ,
  .m_tdata  ( adder_tdata[1]       ) 
);
...
) inst_wr_xpm_fifo_sync[LP_NUM_WRITE_CHANNELS-1:0] (
  .sleep         ( 1'b0             ) ,
  .rst           ( areset           ) ,
  .wr_clk        ( ap_clk           ) ,
  .wr_en         ( adder_tvalid     ) ,
...
// AXI4 Write Master
krnl_ro_rtl_axi_write_master #( 
  .C_ADDR_WIDTH       ( C_M_AXI_GMEM_ADDR_WIDTH ) ,
  .C_DATA_WIDTH       ( C_M_AXI_GMEM_DATA_WIDTH ) ,
  .C_MAX_LENGTH_WIDTH ( LP_LENGTH_WIDTH     ) ,
  .C_BURST_LEN        ( LP_AXI_BURST_LEN        ) ,
  .C_NUM_CHANNELS_WR  ( LP_NUM_WRITE_CHANNELS    ) ,
  .C_LOG_BURST_LEN    ( LP_LOG_BURST_LEN        ) 
)
...
// Those changes needs to be verified as well ////
  .ctrl_start  ( wm_ctrl_start      ) ,         //
  .ctrl_offset ( wm_ctrl_offset     ) ,         //
  .ctrl_length ( wm_ctrl_length     ) ,         //
  .ctrl_done   ( wm_done            ) ,         //
//////////////////////////////////////////////////
